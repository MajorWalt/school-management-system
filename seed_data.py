...
from students.models import Student, StudentStatusLog, Guardian, StudentGuardian, House
...
year, created = AcademicYear.objects.get_or_create(school=school, name="2024-2025", defaults={"is_current": True})
...
house_data = [("Boeri", "Red"), ("Freshwater", "Blue"), ("Trafalgar", "Green"), ("Emerald", "Yellow")]
houses = []
...
for fname, order, hr_names in forms_data:
    ...
    for hr_name in hr_names:
        hr, _ = Homeroom.objects.get_or_create(school=school, name=hr_name, defaults={"form": f})
...
for name, code in course_names:
    c, _ = Course.objects.get_or_create(school=school, name=name, defaults={"code": code, "active": True})
    courses[code] = c
...
for i, (dept, role_title, spec) in enumerate(teacher_specs, start=1):
    ...
    user, _ = User.objects.get_or_create(
        email=email, defaults={"first_name": first, "last_name": last, "password": make_password("teacher123"), "is_active": True}
    )
...
course_teacher_map = {"MATH": 0, "ENG": 1, "SCI": 2, "SS": 3, "PE": 4, "IT": 5, "ART": 6, "FRE": 7}
sections = {}
    ...
    for code, course in courses.items():
        ...
        for term_num in [1, 2, 3]:
            sec, _ = Section.objects.get_or_create(
                school=school, course=course, academic_year=year, term_number=term_num, form=f, defaults={"teacher": teacher, "room": random.choice(hr_names)}
            )
...
for fname, f in forms.items():
    for homeroom in form_homerooms[fname]:
        for _ in range(random.randint(20, 28)):
            ...
            user, _ = User.objects.get_or_create(
                email=email, defaults={"first_name": first, "last_name": last, "password": make_password("student123"), "is_active": True}
            )
            ...
            if created:
                ...
                guardian, _ = Guardian.objects.get_or_create(
                    ...
                    last_name=g_last,
                    phone=fake.dm_phone(),
                    defaults={"email": fake.unique.email(), "address": f"{fake.dm_address()}, {community}"},
                )
                StudentGuardian.objects.get_or_create(
                    student=student, guardian=guardian, defaults={"relationship": "Mother", "is_primary": True, "can_pickup": True}
                )
...
for hr_name, homeroom in homerooms.items():
    ...
    for student in hr_students:
        for day in school_days:
            ...
            Attendance.objects.get_or_create(school=school, student=student, homeroom=homeroom, date=day, defaults={"status": status, "marked_by": admin_user})
            att_count += 1
        ...
        for student in enrolled:
            for ev in evals:
                ...
                _, created = GradeEntry.objects.get_or_create(
                    evaluation=ev, student=student, defaults={"school": school, "marks_earned": marks, "is_absent": is_absent, "entered_by": admin_user}
                )
                if created:
                    grade_count += 1
...
TimetableSettings.objects.update_or_create(school=school, defaults={"mode": CYCLE, "cycle_length": 6, "anchor_date": datetime.date(2024, 9, 3)})
...
for name, order, start, end, is_break in period_data:
    TimetablePeriod.objects.get_or_create(school=school, order=order, defaults={"name": name, "start_time": start, "end_time": end, "is_break": is_break})
...
slot_count = 0
for fname, f in forms.items():
    timetable, _ = Timetable.objects.get_or_create(school=school, form=f, academic_year=year, term_number=1)
    idx = 0
    for day in cycle_days:
        for period in teaching_periods:
            code = subject_codes[idx % len(subject_codes)]
            section = sections[(fname, code, 1)]
            _, created = TimetableSlot.objects.get_or_create(timetable=timetable, day_number=day, period=period, section=section, defaults={"school": school})
            if created:
                slot_count += 1
            idx += 1
    ...
    _, created = TimetableSlot.objects.get_or_create(
        timetable=timetable, day_number=1, period=teaching_periods[-1], section=sections[(fname, "ART", 1)], defaults={"school": school}
    )
    if created:
        slot_count += 1
...
print("  Forms:     Form 1 (101, 102, 103)  |  Form 2 (104, 105, 201)")
print(f"  Houses:    {', '.join(h.name for h in houses)}")
print("  Timetable: 6-day cycle, Term 1 filled for both forms")
print()
