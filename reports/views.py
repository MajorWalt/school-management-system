...
def reports_home(request):
    ...
    if staff and not admin:
        teacher_sections = Section.objects.filter(school=school, teacher=staff).select_related("course", "form", "academic_year")
        teacher_homerooms = Homeroom.objects.filter(school=school, staff_members=staff).select_related("form")
...
def student_roster(request):
    ...
    if as_pdf:
        return _render_pdf(
            request,
            "reports/pdf/student_roster.html",
            {
                "students": students,
                "school": school,
...
def staff_list_report(request):
    ...
    departments = Staff.objects.filter(school=school).values_list("department", flat=True).distinct().exclude(department="").order_by("department")

    as_pdf = request.GET.get("pdf") == "1"
    if as_pdf:
        return _render_pdf(
            request,
            "reports/pdf/staff_list.html",
            {
                "staff": staff,
                "school": school,
                "school_profile": request.school_profile,
...
def class_list(request):
    ...
    if admin:
        ...
        homerooms = Homeroom.objects.filter(school=school, staff_members=staff).select_related("form")
    ...
    if homeroom_pk:
        ...
        if selected_homeroom:
            students = Student.objects.filter(school=school, homeroom=selected_homeroom).select_related("form", "house").order_by("last_name", "first_name")
...
def attendance_summary(request):
    ...
    non_school_days = set(NonSchoolDay.objects.filter(school=school, date__gte=first_day, date__lte=last_day).values_list("date", flat=True))
...
def merit_demerit_report(request):
    ...
    if report_type == "merit":
        records_qs = MeritRecord.objects.filter(school=school, date__gte=first_day, date__lte=last_day).select_related("student", "awarded_by")

        student_totals = {r["student_id"]: r["total"] for r in records_qs.values("student_id").annotate(total=Sum("count"))}
        student_records = {}
        ...
        records_qs = DemeritRecord.objects.filter(school=school, date__gte=first_day, date__lte=last_day).select_related("student", "awarded_by")
...
def grade_by_course(request):
    ...
    if section_pk:
        selected_section = Section.objects.filter(pk=section_pk, school=school).select_related("course", "form", "academic_year", "teacher").first()

        if selected_section:
            evaluations = Evaluation.objects.filter(school=school, section=selected_section).order_by("date", "created_at")

            enrolments = Enrolment.objects.filter(section=selected_section).select_related("student").order_by("student__last_name", "student__first_name")

            entries = GradeEntry.objects.filter(school=school, evaluation__section=selected_section).select_related("evaluation", "student")
...
def grade_by_student(request):
    ...
    if selected_student and year_pk:
        ...
        for enrolment in enrolments:
            ...
            evaluations = Evaluation.objects.filter(school=school, section=section).order_by("date", "created_at")

            entries = GradeEntry.objects.filter(school=school, evaluation__section=section, student=selected_student).select_related("evaluation")
            grade_map = {(e.evaluation_id, e.student_id): e for e in entries}
...
def teacher_gradebook(request):
    ...
    sections = Section.objects.filter(school=school).select_related("course", "form", "academic_year", "teacher").order_by("teacher__last_name", "course__name")
...
def grade_overview(request):
    ...
    selected_form = forms.filter(pk=form_pk).first() if form_pk else None
    selected_homeroom = homerooms.filter(pk=homeroom_pk).first() if homeroom_pk else None
    selected_year = AcademicYear.objects.filter(pk=year_pk, school=school).first() if year_pk else None
    ...
    if selected_year and (selected_form or selected_homeroom):
        ...
        section_filter = Section.objects.filter(school=school, academic_year=selected_year)
        if selected_homeroom:
        ...
        section_filter = section_filter.select_related("course").order_by("course__sequence", "course__name")
        ...
        all_evals = Evaluation.objects.filter(school=school, section__in=section_filter).select_related("section__course")
        ...
        for student in students_list:
            ...
            for sec in section_filter:
                ...
                if cname in course_avgs and course_avgs[cname] is not None and avg is not None:
                    course_avgs[cname] = round((course_avgs[cname] + avg) / 2, 1)
                else:
