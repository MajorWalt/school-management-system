...
def student_detail(request, pk):
    ...
    if current_year is None:
        current_year = AcademicYear.objects.filter(school=request.school).order_by("-start_date", "-name").first()
        pass
...
def student_status_change(request, pk):
    ...
    return render(
        request,
        "students/student_status_form.html",
        {
            "form": form,
            "student": student,
        },
...
def student_withdraw(request, pk):
    ...
    form = WithdrawForm(request.POST or None, initial={"withdraw_date": datetime.date.today().isoformat()})
