...
from .models import Guardian, Student, StudentGuardian, StudentStatusLog
...
class BulkEnrolForm(forms.Form):
    excel_file = forms.FileField(label="Excel File (.xlsx)", help_text="Download the template, fill it in, then upload here.")
...
class WithdrawForm(forms.Form):
    withdraw_date = forms.DateField(label="Withdraw Date", widget=forms.DateInput(attrs={"type": "date", "class": FIELD_CLASS}))
    reason = forms.CharField(
        label="Withdraw Reason", max_length=200, widget=forms.TextInput(attrs={"class": FIELD_CLASS, "placeholder": "e.g. Relocated, Family decision..."})
    )
    description = forms.CharField(
        label="Description", required=False, widget=forms.Textarea(attrs={"class": FIELD_CLASS, "rows": 3, "placeholder": "Additional details..."})
    )
