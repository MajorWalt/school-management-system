...
def can_do_attendance(user, school):
    ...
    if "teacher" in roles:
        ...
        try:
            ...
            return Homeroom.objects.filter(school=school, staff_members=staff).exists()
        except Exception:
            return False
    return False
...
def attendance_home(request):
    ...
    for hr in homerooms:
        ...
        absences_today = Attendance.objects.filter(school=request.school, homeroom=hr, date=today, status="absent").count()
        last_marked = Attendance.objects.filter(school=request.school, homeroom=hr).order_by("-date").values_list("date", flat=True).first()
...
def homeroom_select(request, date):
    ...
    homeroom_status = []
    for hr in homerooms:
        marked = Attendance.objects.filter(school=request.school, homeroom=hr, date=mark_date).exists()
        homeroom_status.append({"homeroom": hr, "marked": marked})

    return render(
        request,
        "attendance/homeroom_select.html",
        {
...
def attendance_report(request, homeroom_pk):
    ...
    student_summary = []
    for student in students:
        absences = Attendance.objects.filter(school=request.school, homeroom=homeroom, student=student, status="absent").count()
        lates = Attendance.objects.filter(school=request.school, homeroom=homeroom, student=student, status="late").count()
        excused = Attendance.objects.filter(school=request.school, homeroom=homeroom, student=student, status="excused").count()
