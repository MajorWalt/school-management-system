...
def build_report_card(school, student, academic_year, up_to_term):
    ...
    terms = list(TermConfig.objects.filter(academic_year=academic_year, term_number__lte=up_to_term).order_by("term_number"))
...
def conduct_by_term(school, student, terms):
    ...
    for t in terms:
        ...
        if s and e:
            merits = MeritRecord.objects.filter(school=school, student=student, date__gte=s, date__lte=e).aggregate(x=Sum("count"))["x"] or 0
            demerits = DemeritRecord.objects.filter(school=school, student=student, date__gte=s, date__lte=e).aggregate(x=Sum("count"))["x"] or 0
