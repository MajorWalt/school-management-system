...
def merit_list(request):
    ...
    if query:
        merits = merits.filter(student__first_name__icontains=query) | merits.filter(student__last_name__icontains=query)
        demerits = demerits.filter(student__first_name__icontains=query) | demerits.filter(student__last_name__icontains=query)

    return render(
        request,
        "merits/merit_list.html",
        {
            "merits": merits,
            "demerits": demerits,
            "query": query,
...
def student_merit_report(request, student_pk):
    ...
    # Monthly breakdown
    merit_monthly = merits.annotate(month=TruncMonth("date")).values("month").annotate(total=Sum("count")).order_by("-month")
    demerit_monthly = demerits.annotate(month=TruncMonth("date")).values("month").annotate(total=Sum("count")).order_by("-month")

    return render(
        request,
        "merits/student_report.html",
        {
            "student": student,
            "merits": merits,
            "demerits": demerits,
...
def school_summary(request):
    ...
    merit_monthly = (
        MeritRecord.objects.filter(school=request.school).annotate(month=TruncMonth("date")).values("month").annotate(total=Sum("count")).order_by("-month")
    )
    demerit_monthly = (
        DemeritRecord.objects.filter(school=request.school).annotate(month=TruncMonth("date")).values("month").annotate(total=Sum("count")).order_by("-month")
    )
