...
def timetable_settings(request):
    ...
    if request.method == "POST":
        ...
        pass
    return render(request, "scheduling/timetable_settings.html", {"form": form, "settings": settings})
    pass
...
def manage_periods(request):
    ...
    return render(request, "scheduling/manage_periods.html", {"form": form, "periods": periods})
    pass
...
def cycle_calendar(request):
    ...
    return render(
        request,
        "scheduling/cycle_calendar.html",
        {
            "weeks": weeks,
...
def _term_options(school):
    ...
    terms = TermConfig.objects.filter(academic_year__school=school).select_related("academic_year").order_by("academic_year__name", "term_number")
    out = []
    for t in terms:
...
def timetable_grid(request):
    ...
    forms_qs = Form.objects.filter(school=school).order_by("name") if hasattr(Form, "name") else Form.objects.filter(school=school)
    ...
    if request.method == "POST" and timetable:
        ...
        if action == "remove":
            TimetableSlot.objects.filter(pk=request.POST["slot_id"], timetable=timetable).delete()
        elif action == "add":
    ...
    if timetable:
        ...
        available_sections = Section.objects.filter(school=school, form=selected_form, academic_year_id=year_id, term_number=term_number).select_related(
            "course", "teacher"
        )

        placed = TimetableSlot.objects.filter(timetable=timetable).select_related("section", "section__course", "section__teacher")
        cell_map = {}
        for slot in placed:
    ...
    return render(
        request,
        "scheduling/timetable_grid.html",
        {
            "settings": settings,
            "forms": forms_qs,
            "term_opts": term_opts,
...
def timetable_copy(request, pk):
    ...
    sources = Timetable.objects.filter(school=school).exclude(pk=target.pk).select_related("form", "academic_year")
    ...
    return render(request, "scheduling/timetable_copy.html", {"target": target, "sources": sources})
    pass
