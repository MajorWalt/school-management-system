...
def marquee_content(request):
	...
	if school:
		non_school_day = NonSchoolDay.objects.filter(
			school=school, date=today
		).first()
		
		if non_school_day:
			# Format: "No school — Holiday Label · Day, Date"
			type_label = dict(NonSchoolDay.TYPE_CHOICES).get(
				non_school_day.type, non_school_day.type
			)
	...
	return {
		...
		"today": today,
		"is_non_school_day": school and NonSchoolDay.objects.filter(
			school=school, date=today
		).exists(),
	}
