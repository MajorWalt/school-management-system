...
def section_roster(request, pk):
    ...
    if request.method == "POST":
        ...
        if action == "remove":
            Enrolment.objects.filter(section=section, pk=request.POST.get("enrolment")).delete()
        ...
        if action == "remove_homeroom":
            Enrolment.objects.filter(section=section, source_homeroom_id=request.POST.get("homeroom")).delete()
    ...
    # --- GET: build the page ---
    enrolments = (
        Enrolment.objects.filter(section=section)
    ...
    if pick_hr_id:
        ...
        if pick_homeroom is not None:
            ...
            pick_students = _active_students(school, pick_homeroom).exclude(pk__in=enrolled_ids).order_by("last_name", "first_name")
            pass
        pass

    return render(
        request,
        "scheduling/section_roster.html",
        {
            "section": section,
